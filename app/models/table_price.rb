...
class TablePrice < ApplicationRecord
  ...
  def dimensioning
    self.max_dimension = (max_length.to_d * max_width.to_d * max_height.to_d) / 1_000_000
    self.minimum_dimension = (minimum_length.to_d * minimum_width.to_d * minimum_height.to_d) / 1_000_000
  end
  ...
  def nulling_interval_conflict
    ...
    tp.each do |t|
      ...
      if minimum_dimension >= t.minimum_dimension && minimum_dimension <= t.max_dimension
        errors.add(:minimum_dimension, 'deve possuir valor maior ou inferior ao dos intervalos anteriores')
      elsif minimum_dimension < t.minimum_dimension && max_dimension > t.minimum_dimension
        errors.add(:minimum_dimension, 'não pode possuir intervalos que abrangem outros intervalos')
      end

      if max_dimension > t.minimum_dimension && max_dimension < t.max_dimension
        errors.add(:max_dimension, 'deve possuir valor maior ou inferior ao dos intervalos anteriores')
        errors.add(:max_dimension, 'não pode possuir intervalos que abrangem outros intervalos')
      end
    end
  end
end
