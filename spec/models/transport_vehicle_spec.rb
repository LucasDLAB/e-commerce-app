...
RSpec.describe TransportVehicle, type: :model do
  describe '#valid' do
    ...
    it 'falso se o campo Ano de fabricação for futura' do
      vehicle = described_class.create(brand: 'Mercedes', year_manufacture: Time.zone.now.year + 1, payload: 9000,
      ...
      expect(vehicle.errors[:year_manufacture]).to include("deve ser menor ou igual a #{Time.zone.now.year}")
    end

    it 'falso se o campo Capacidade máxima deve ser maior que 0' do
      vehicle = described_class.create(brand: 'Mercedes', year_manufacture: Time.zone.now.year, payload: 0,
                                       identification_plate: 'AAAA000', vehicle_model: 'Atego',
                                       height: 12, length: 33, width: 43)
    ...
    it 'falso se o campo Placa de identificação não for único' do
      ...
      described_class.create(brand: 'Mercedes', year_manufacture: Time.zone.now.year, payload: 10,
                             identification_plate: 'AAAA000', vehicle_model: 'Atego',
                             height: 12, length: 33, width: 43, shipping_company_id: shipping_company.id)
      second_vehicle = described_class.create(brand: 'Mercedes', year_manufacture: Time.zone.now.year, payload: 10,
                                              identification_plate: 'AAAA000', vehicle_model: 'Atego',
                                              height: 12, length: 33, width: 43, shipping_company_id: shipping_company.id)
