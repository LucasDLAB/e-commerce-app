...
RSpec.describe EstimatedDate, type: :model do
	describe "#valid" do
		...
		it "falso se os campos não forem números" do
			...
			ed = EstimatedDate.create(min_distance:"A",max_distance:"A",business_day:"A",
		...
		it "falso se os campos forem menores ou iguais à zero" do
			shipping_company = create(:shipping_company)
			ed = EstimatedDate.create(min_distance:0,max_distance:0,business_day:0,shipping_company_id:shipping_company.id)
		...
		it "falso se Distância máxima menor que Distância mínima" do
			...
			ed = EstimatedDate.create(min_distance:10,max_distance:5,business_day:1,shipping_company_id: shipping_company.id)
		...
		it "verdadeiro se o campo Distância máxima e Distância mínima for igual em Tabelas de diferentes transportadoras" do 	
			...
			ed = EstimatedDate.create!(min_distance:5,max_distance:10,business_day:1,shipping_company_id: first_company.id)
			
			second_ed = EstimatedDate.create(min_distance:5,max_distance:10,business_day:1,shipping_company_id: second_company.id)
		...
		it "falso se as Distãncia de estimativa possuir intervalos intercalados" do
			shipping_company = create(:shipping_company)
			ed = EstimatedDate.create(min_distance:5,max_distance:10,business_day:1,shipping_company_id:shipping_company.id)
			second_ed = EstimatedDate.create(min_distance:2,max_distance:7,business_day:1,shipping_company_id:shipping_company.id)
		...
		it "falso se Distãncia de estimativa estiver entre intervalos" do
			shipping_company = create(:shipping_company)
			ed = EstimatedDate.create(min_distance:5,max_distance:10,business_day:1,shipping_company_id: shipping_company.id)
			second_ed = EstimatedDate.create(min_distance:6,max_distance:11,business_day:1,shipping_company_id:shipping_company.id)
